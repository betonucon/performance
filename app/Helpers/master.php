...
function akumulasi_target($id){
    ...
    $detail=App\Target::where('deployment_id',$id)->get();
...
function akumulasi_capaian($id){
    ...
    $jumlah=App\Target::where('deployment_id',$id)->where('realisasi','!=',0)->count();
        ...
        if($data['rumus_akumulasi']==3){
            $detailcapaian=App\Target::where('deployment_id',$id)->where('realisasi','!=',0)->orderBy('bulan','desc')->firstOrFail();
