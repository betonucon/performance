                ...
                <div class="box-body" style="padding:10px">
                        ...
                        <div class="mailbox-controls" style="background:#f8f8fb;margin-bottom:10px;margin-top:5px;margin-bottom:20px">
                            ...
                            @if(Auth::user()['role_id']==1 || Auth::user()['role_id']==3 )
                                @if(validasi_all($tahun)>0)
                                <span   class="btn btn-success btn-sm" id="validasi_all"  onclick="validasi_all({{$tahun}})" style="margin-left:5px;margin-top:2px" ><i class="fa fa-check"></i> All Validasi</span>
                                @endif
                            @endif
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @foreach(deployment_target($kode,$tahun) as $no=>$data)
                                    <tr>
                                        <td>{{$no+1}}</td>
                                        ...
                                        <td>{{$data->tahun}}-{{$data['status_id']}}</td>
...
<script>
    ...
    function validasi_all(a){
        $.ajax({
            type: 'GET',
            url: "{{url('/target/validasi_all')}}/"+a,
            data: "id="+a,
            beforeSend: function(){
                $('#validasi_all').val('');
            },
