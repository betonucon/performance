                    ...
                    @if(Auth::user()['role_id']==1)
                        <div class="mailbox-controls" style="text-align:center;background:#e5e5f5;">
                            @if($data['status_id']==1 || $data['status_id']==2)
                                ...
                                @if($data['status_id']==2)
                                <span class="btn btn-primary btn-sm" id="simpan_selesai_target" onclick="simpan_selesai_target()">Selesai</span>
                                @endif
...
<script>
    ...
    function simpan_selesai_target(){
        var form=document.getElementById('mytarget');
        var a="{{$id}}";
            $.ajax({
                type: 'POST',
                url: "{{url('/target/simpan_selesai_target')}}/"+a,
                data: new FormData(form),
                contentType: false,
                cache: false,
                processData:false,
